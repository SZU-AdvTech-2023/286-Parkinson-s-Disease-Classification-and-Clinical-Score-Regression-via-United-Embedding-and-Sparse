function [fsAcc,fsSen,fsSpec,fsPrec,maxFscore,fsAuc,maxAcc,AccRmse,maxBcc,BccRmse,maxCcc,CccRmse,maxDcc,DccRmse,Rs,Ra,Rb,Rc,Rd]=...
    SVM_SVR(trainData,trainFlag,trainScoresA,trainScoresB,trainScoresC,trainScoresD,...
...
for i=1:lparc
    for j=1:lparg
        ...
        svrDataA{i,j} = getSVRData(trainData,trainScoresA,testData,testScoresA,cmdSVR1);
        ...
        svrDataB{i,j} = getSVRData(trainData,trainScoresB,testData,testScoresB,cmdSVR1);
        ...
        svrDataC{i,j} = getSVRData(trainData,trainScoresC,testData,testScoresC,cmdSVR1);
        ...
        svrDataD{i,j} = getSVRData(trainData,trainScoresD,testData,testScoresD,cmdSVR1);
        ...
        svmData{i,j} = getSVMData(trainData,trainFlag,testData,testFlag,cmdSVM);
...
[maxAcc,AccRmse,AccDec,AccTest,AccBest_X,AccBest_Y]=getBestCcSVR(Acc,Armse,Adec,Atest);

[maxBcc,BccRmse,BccDec,BccTest,BccBest_X,BccBest_Y]=getBestCcSVR(Bcc,Brmse,Bdec,Btest);

[maxCcc,CccRmse,CccDec,CccTest,CccBest_X,CccBest_Y]=getBestCcSVR(Ccc,Crmse,Cdec,Ctest);

[maxDcc,DccRmse,DccDec,DccTest,DccBest_X,DccBest_Y]=getBestCcSVR(Dcc,Drmse,Ddec,Dtest);

[fsAcc,fsSen,fsSpec,fsPrec,fsAuc,maxFscore,fsLdec,fsLtest,fsTP,fsFP,fsBest_X,fsBest_Y]=...
    getBestFsSVM(SVMacc,SVMsen,SVMspec,SVMprec,SVMauc,SVMfscore,SVMldec,SVMltest,SVMtp,SVMfp);
